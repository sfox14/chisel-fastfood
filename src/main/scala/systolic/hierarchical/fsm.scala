...
class FSM( val bitWidth : Int, val fracWidth : Int, 
  ...
  val pcBL = RegInit( UInt(0, width=log2Up( log2Up( b ) )+2 ) )
  ...
  val latency_extra = h + (aStages+1+1) + (aStages+1) + (log2Up(b) + 2)
  ...
  when( global === UInt(1) ){
    ...
    when( state === UInt(6) ){
      ...
      dFunc := UInt(6)
      pcBL := pcBL + UInt(1)
      // wait log2Up( b ) cycles for adder tree and 2 cycle for sum_local
      when( pcBL === UInt( log2Up( b ) - 1 + 2 ) ){
    ...
    pcL := pcL + UInt(1)
    when( pcL === UInt( max(latency - d, 0).toInt ) ){
      loading := Bool(true)
    }
  }
  ...
  io.xrdy := ShiftRegister( ready, mStages - 1 + bcy )
  io.yrdy := ShiftRegister( outReady, 1 + mStages + eStages + aStages + 1 + bcy ) //fsm to aluCode

  io.ctrl.pe.pload := ShiftRegister( pLoad, mStages - 1 + bcy )
  io.ctrl.pe.padd := pAdd 
  io.ctrl.pe.func := ShiftRegister( dFunc, bcy )
  io.ctrl.pe.sx := ShiftRegister( sX, mStages + eStages + aStages + bcy ) //delay through pipeline, mStages + 1 + aStages + 1
  
  io.ctrl.pe.yload := ShiftRegister( yLoad, mStages - 1 + bcy )

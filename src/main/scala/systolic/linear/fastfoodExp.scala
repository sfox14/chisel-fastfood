...
class Fastfood( val bitWidth : Int, val fracWidth : Int, 
  ...
  val array = (0 until p).reverse.map( i => Module( new PE(i, bitWidth, fracWidth, 
                                                    n, p, d, g(i), s(i), alpha(i), 
                                                    aStages, cosFlag, forSim ) ) )

  val fsm = Module( new FSM( bitWidth, fracWidth, n, p, d, aStages, cosFlag ) )
...
object fastfoodVerilog{
  ...
  def main(args: Array[String]): Unit = {
    ...
    chiselMain(Array("--backend", "v", "--targetDir", "verilog"), 
                () => Module( new Fastfood( bitWidth, fracWidth, n, p, d, g, s, alpha, 
                  aStages, true, false ) ) )
  }

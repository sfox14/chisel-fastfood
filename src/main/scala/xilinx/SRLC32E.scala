...
class SRLC32E( val hex : String, val len : Int ) extends BlackBox {
...
class SRLC32E_sim( val hex : String, val len : Int ) extends Module {
...
class SRLC32E_inst extends Module {
  ...
  val len = 32
  val srl = Module( new SRLC32E( "0000000", len ) )
  srl.io.dIn := io.a

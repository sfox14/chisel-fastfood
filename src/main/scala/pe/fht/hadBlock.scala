...
class HadBlock( val bitWidth : Int, val fracWidth : Int, val n : Int, 
	...
	val sumMod = Module( new SumMod( bitWidth, fracWidth, b ) )
	...
	sumMod.io.pload := io.ctrl.pe.pload
...
class SumMod( val bitWidth : Int, val fracWidth : Int, val b : Int ) extends Module {
	...
	// Module for computing local sum, global sum, error, and back to PEs

	// register control signal
	val pload = Reg( init=Bool(false), next=io.pload )
	...
	val eta = RegInit( Fixed(BigInt(6), bitWidth, fracWidth) )
	sreg := io.gsin
	...
	y_parr := io.yin
	val y_data = RegInit( Fixed(0, width=bitWidth, fracWidth=fracWidth) )
	when( pload ){
		y_data := y_parr
	}
	// sometime before the two ploads
	when( aluCode === UInt(2) ){
		yreg := -y_data	
	}

...
class FSM( val bitWidth : Int, val fracWidth : Int, 
  ...
  val oCycles = 4 // regIn + regIn + regOut
  val dCycles = oCycles + (n/p) - 2 // cycles that dpath2 starts ahead of pLoad
  val iCycles = max( oCycles + 1, (n/p)+1 ) // stages between dFunc=0 and dFunc=3 
  val sCycles = 20 // stall dpath1, parameterise this
  val regOut = 2
  ...
  val state = RegInit( UInt(0, width=3) ) //dpath2
  ...
  io.rdy := ShiftRegister( ready, dCycles )


  io.ctrl.prst := ShiftRegister( RegNext(pRst), dCycles )
  io.ctrl.pload := ShiftRegister( pLoad, dCycles )
  io.ctrl.pinc := ShiftRegister( RegNext(pInc), dCycles - regOut - 1 )
  io.ctrl.padd := ShiftRegister( RegNext(pAdd), dCycles - regOut )
  io.ctrl.dload := dLoad
  io.ctrl.dadd := dAdd
  io.ctrl.func := dFunc

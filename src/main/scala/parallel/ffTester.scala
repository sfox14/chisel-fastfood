...
class ffSim( c: FF, X : List[List[BigInt]], verbose : Boolean = true ) extends Tester(c) {

  val rng = new Random( 23 )

  val f = new java.io.File( "ff_ypred.csv" )
      ...
      */
      ...
      while ( !en ){
        ...
        if ( (peek(c.io.dOut.valid) == 1) && numI < num ){   
          var a = Buffer[Double]()
          a += fromPeek.toDbl( peek(c.io.dOut.bits), c.bitWidth, c.fracWidth )
          println( a )
          writer.writeRow( a )
          numI += 1
